when defined(useBoringSSL):
  ...
  proc reqTask(p: int): CpsFuture[(Http3ClientResponse, Http3ClientResponse)] {.cps.} =
    let emptyHeaders: seq[(string, string)] = @[]
    let auth = "127.0.0.1:" & $p
    let r1 = await doHttp3Request(host="127.0.0.1", port=p, meth="GET", path="/", authority=auth, headers=emptyHeaders, body="")
    let r2 = await doHttp3Request(host="127.0.0.1", port=p, meth="POST", path="/", authority=auth, headers=emptyHeaders, body="nim-live-post-body")
    return (r1, r2)
  ...
  if f.hasError:
    ...
    let (a,b) = f.read()
    echo "OK1=", a.statusCode, " ", a.body
    echo "OK2=", b.statusCode, " ", b.body
