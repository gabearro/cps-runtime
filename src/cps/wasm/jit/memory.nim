...
## Handles mmap with W^X protection for macOS ARM64 (Apple Silicon)

when not defined(macosx):
  {.error: "JIT memory currently only supports macOS".}
...
const
  ...
  MAP_PRIVATE = 0x0002
  MAP_ANON = 0x1000
  MAP_JIT = 0x0800  # macOS: allow JIT code generation
...
proc writeCode*(pool: var JitMemPool, code: openArray[uint32]): JitCode =
  ## Write a sequence of 32-bit ARM instructions to the pool
