...
proc emitBridgeNimShim*(irProgram: GuiIrProgram): string =
  ...
  lines.add "  ## Binary wire format: [u32 actionTag][u32 stateLen][stateBlob...]"
  lines.add "  let stateLen = payload.stateBlob.len"
  lines.add "  result = newSeq[byte](8 + stateLen)"
  lines.add "  let actionVal = ord(payload.actionTag).uint32"
  lines.add "  result[0] = byte(actionVal and 0xFF'u32)"
  lines.add "  result[1] = byte((actionVal shr 8) and 0xFF'u32)"
  lines.add "  result[2] = byte((actionVal shr 16) and 0xFF'u32)"
  lines.add "  result[3] = byte((actionVal shr 24) and 0xFF'u32)"
  lines.add "  let lenVal = stateLen.uint32"
  lines.add "  result[4] = byte(lenVal and 0xFF'u32)"
  lines.add "  result[5] = byte((lenVal shr 8) and 0xFF'u32)"
  lines.add "  result[6] = byte((lenVal shr 16) and 0xFF'u32)"
  lines.add "  result[7] = byte((lenVal shr 24) and 0xFF'u32)"
  lines.add "  if stateLen > 0:"
  lines.add "    copyMem(addr result[8], unsafeAddr payload.stateBlob[0], stateLen)"
  lines.add ""
  ...
  lines.add "    result.actionTag = low(GuiBridgeActionTag)"
  lines.add ""
  lines.add "  let stateLen ="
  lines.add "    uint32(blob[4]) or (uint32(blob[5]) shl 8) or"
  lines.add "    (uint32(blob[6]) shl 16) or (uint32(blob[7]) shl 24)"
  lines.add "  let needed = 8 + stateLen.int"
  lines.add "  if stateLen > 0 and blob.len >= needed:"
  lines.add "    result.stateBlob = newSeq[byte](stateLen.int)"
  lines.add "    copyMem(addr result.stateBlob[0], unsafeAddr blob[8], stateLen.int)"
...
proc emitBridgeImplTemplate*(irProgram: GuiIrProgram): string =
  ...
  lines.add "  let debugMsg = \"bridge dispatch actionTag=\" & $ord(decoded.actionTag) & \" stateBytes=\" & $decoded.stateBlob.len"
